import ScrollAnimation from "react-animate-on-scroll";

interface TwoBlock1 {
  title: string;
  description: string;
  projUrl: string;
  imageUrl: string;
  ctaText: string;
  extraHtmlText?: any;
}
export default function TwoBlock1({ title, description, projUrl, ctaText, imageUrl, extraHtmlText }: TwoBlock1) {
  return (
    <div className="relative bg-gray-50">
      <section className="relative pt-12 sm:pt-16 lg:pt-20 pb-0 animate__fadeIn animate__animated">
        <div className="px-4 mx-auto sm:px-6 lg:px-12 max-w-6xl overflow-hidden max-h-fit">
          <div className="grid grid-cols-1 gap-y-8 md:grid-cols-2 sm:gap-y-20 xl:grid-cols-5">
            <ScrollAnimation
              animateOnce
              className="xl:col-span-2 text-left md:px-8"
              animateIn="animate__fadeInLeft animate__animated"
            >
              <div className="lg:mx-auto lg:max-w-full">
                <h1 className="text-2xl font-bold leading-tight text-gray-900 sm:leading-tight lg:text-3xl lg:leading-tight ">
                  {title}
                </h1>
                <p className="">{projUrl}</p>
                <p className="mt-4 text-lg text-gray-900">{description}</p>
              </div>
              <div className="mt-4">
                <a
                  href="#"
                  title=""
                  className="inline-flex items-center justify-center px-6 p-2 text-md text-orange-400 transition-all duration-200 border rounded-md border-orange-400"
                  role="button"
                >
                  {ctaText}
                </a>
              </div>
              <div className="lg:mx-auto lg:max-w-full">{extraHtmlText}</div>
            </ScrollAnimation>
            <ScrollAnimation
              animateOnce
              style={{ maxHeight: "600px" }}
              className="xl:col-span-3 p-0 lg:p-12 lg:pt-0"
              animateIn="animate__fadeInRight animate__fadeIn animate__animated"
            >
              <img src={imageUrl} alt="" className="shadow-2xl shadow-slate-600" />
            </ScrollAnimation>
          </div>
        </div>
      </section>
    </div>
  );
}
